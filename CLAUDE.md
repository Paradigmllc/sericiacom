...
| **F37 全ページ共通くるくる + Cache Rules** | クリックして5秒無反応問題（UX致命傷）の二段同時打: ① 全ページ共通loading + ② edge cache | ✅ 完了 (this commit) | `b75ee159`(worktree) → `1a546183`(main) | **User report 2026-04-30**: 「メニューやロゴをクリックしてもページ遷移に5秒以上かかる、しかも一切ローディングアニメーションがないからUX最悪」「全ページローディングアニメーション（くるくる）がない、全ページ共通デザインで実装して」/ **Root cause #1**: ZERO `loading.tsx` files in App Router tree → no Suspense fallback fires when navigation triggers slow server-side render → 5+秒間 UI完全停止 / **Root cause #2**: `RouteProgress` の useEffect が `useEffect([pathname])` で 発火していたが、App Router の `usePathname()` は新ルート解決**完了後**に更新される仕様 → progress bar は遷移**後**に表示される実装になっていた + 380ms 強制終了タイマーで progress bar が遷移完了前に消える二重バグ / **Fix 1 (Layer 1: Suspense via loading.tsx)**: `components/PageLoading.tsx`（shared spinner・single source of truth・`tone="paper"|"bare"` 2variants・centered SVG ring・90% arc・900ms cubic-bezier rotation・prefers-reduced-motion safe）+ 3-tier `loading.tsx` (`app/loading.tsx` root fallback / `app/(frontend)/loading.tsx` paper tone / `app/(payload)/loading.tsx` bare tone) → 全route segmentに自動Suspense境界配置 / **Fix 2 (Layer 2: RouteProgress rewrite)**: document-level click delegation で `<a href>`タップ瞬間に発火（modifier-key/target=_blank/mailto/hash-only/download除外）+ asymptotic 8→85% creep（never lies about completion）+ pathname/searchParams commit で 100% snap+280ms fade + 12s safety net for cancelled/same-route nav / **2-layer composition**: クリック0ms → top hairline + corner ring spinner appear instantly (Layer 2) → ~80ms RSC streaming starts → centered ring in main slot (Layer 1) → RSC resolves → 両layer fade → 新ページcommit / **共通視覚言語**: LuxuryLoader（first-paint corner spinner）/ RouteProgress（top hairline + corner ring）/ PageLoading（centered ring）すべて `#21231d` ink stroke + 90% arc + 900ms cubic-bezier rotation で統一 → Aesop/Le Labo級の視覚coherence / **Fix 3 (Cloudflare Cache Rules)**: `scripts/apply-cloudflare-cache-rules.mjs` を sericia.com Cache Rules専用token (memory: `~/.claude/projects/.../reference_api_keys.md` 参照) で実行 → 3 rules適用 (① Dynamic bypass `/api/`,`/cms/`,`/account/`,`/checkout`,`/cart` ② Static assets 30d edge+browser TTL `_next/static/`,`/placeholders/`,`*.svg`,`*.woff2`,`favicon.ico` ③ Storefront HTML 2min edge+1h SWR `/`,`/products`,`/products/*`,`/journal`,`/journal/*`,`/articles/*`,`/guides`,`/guides/*`,`/tools`,`/tools/*`,`/about`,`/shipping`,`/refund`,`/terms`,`/privacy`,`/accessibility`,`/faq`,`/sitemap`,`/tokushoho`) / **Cache hit verification**: `/products` 1st req `MISS` → 3秒後 `HIT, Age:3` / `/api/products/search-index` `DYNAMIC` (correct bypass) / `/favicon.ico` `HIT, max-age=2592000` (30d) → **5秒の origin render は edge cache 命中で ~30ms に短縮**、SWR で編集後も常に visitor は instant 体感 / **net UX**: 0ms click → 即時くるくる feedback (Layer1+2) → edge serves cached HTML in 30ms → new page renders / **Coolify deploy**: `zckxmwd46pga9r2zby2t69yu` (this commit polling until finished) |
