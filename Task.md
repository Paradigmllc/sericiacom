...
|--------|-------|------|--------|
| 2026-05-10 | claude-code | **F58 Stripe direct primary rail — ✅ 本番 LIVE** (fully autonomous, operator 操作ゼロ). 5 deploy 試行で確立: d1=apiVersion literal mismatch / d3=`Stripe.errors.StripeError` class-vs-type / d4=exit 255 OOM (host-level transient・F42 precedent) / d5 (`yiz0m29k9mywtmczluv596mi`)=finished. Stripe API webhook=enabled/livemode + Apple Pay/Google Pay/Link/PayPal active + sericia.com endpoints 401/400/404/200 全グリーン. 同 build で **モバイルヘッダー右クラスタの overflow 修正** (gap/padding shrink + ThemeToggle <md hidden) 同梱. | `6cddf3e3` (F58) → `cc89c5ff` (smoke test) → `2eac4059` (Task.md) → `e8f0ff6c` (mobile UI + apiVersion) → `51a73859` (StripeError type fix) on main |
