# Sericia — Implementation Details & History

> **CEP rule (F69)**: this file holds the implementation-history detail that was previously inlined in `CLAUDE.md` sections s14 (壁打ちメモ) and s15 (M1-F69 execution tracker). It exceeds the 60KB CLAUDE.md target so it lives here under `docs/knowledge/` per the Content Externalization Protocol. `CLAUDE.md` now references this file via short summary + link instead of inlining.
>
> **What's in here**:
> - Section 14 — design-decision rationale (情報非対称, supplier tiers, EMS bracket optimisation, decision history)
> - Section 15 — F1-F69 execution tracker (every milestone, commit, root-cause analysis, postmortem)
>
> **When to read this file**:
> - When you need the "why" behind a current decision (s14)
> - When you're investigating a regression that touches an earlier F-numbered milestone (s15)
> - When you need to find the commit + Coolify deploy UUID for a past launch step
> - When designing a new feature that has similar tradeoffs to a previous one
